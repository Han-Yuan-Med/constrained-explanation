...
class alex_grad_cam(nn.Module):
    def __init__(self, alex_model):
        super().__init__()
        # get the pretrained VGG19 network
        self.alex = alex_model
        # dissect the network to access its last convolutional layer
        self.features_conv = self.alex.features
        # get the max pool of the features stem
        self.max_pool = nn.AdaptiveAvgPool2d(output_size=(6, 6))
        # get the classifier of the vgg19
        self.classifier = self.alex.classifier
...
def grad_cam_mul(model, image, cat_id):
...
def explain_cases_mul_baseline(test_loader_seg, device, cls_model, threshold, cat_id):
    ...
    for data_test in tqdm(test_loader_seg):
        ...
        if labels_test.max() == 0:
            ...
            binary_list_iou = grad_cam_mul(model=cls_model, image=images_test, cat_id=cat_id)
