...
cv2.imwrite('Figure\\GC\\ATE-GC-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\ATE-GC-Mask.png')
cons = cv2.imread(cons_path + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\GC\\ATE-GC-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\ATE-GC-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\ATE-GC-PRO.png')
