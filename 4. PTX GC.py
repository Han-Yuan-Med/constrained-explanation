...
mask_path = "D:\\ChestX-Det10-Dataset-Mask\\Pneumothorax"
cons_path = "D:\\ChestX-Det-Dataset-Lung-Top2"

test_csv = pd.read_csv("Dataset\\mul_test.csv")
test_csv = test_csv.iloc[np.where(test_csv.loc[:, 'ptx'] == 1)]
...
iou_list_test_case, dice_list_test_case, iou_case, dice_case = \
    explain_cases_baseline(test_loader_seg=test_cases_loader_bsl, device=device,
                           cls_model=vgg_grad_cam_11(VGG11_optimal), threshold=95)
...
idx = np.argsort(np.asarray(iou_list_test_case_msk).astype(np.float32)-
          np.asarray(iou_list_test_case).astype(np.float32))[-2]
...
cv2.imwrite('Figure\\GC\\ptx-GC-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\ptx-GC-Mask.png')
cons = cv2.imread(cons_path + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\GC\\ptx-GC-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    binary_list_iou = grad_cam_org(model=vgg_grad_cam_11(VGG11_optimal), image=images_test)
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\ptx-GC-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\ptx-GC-PRO.png')
