...
cv2.imwrite('Figure\\SM\\fra-SM-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\SM\\fra-SM-Mask.png')
cons = cv2.imread('D:\\ChestX-Det-Dataset-Lung-Top2' + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\SM\\fra-SM-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\SM\\fra-SM-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\SM\\fra-SM-PRO.png')
