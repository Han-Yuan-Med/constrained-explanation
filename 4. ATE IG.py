...
mask_path = "D:\\ChestX-Det10-Dataset-Mask\\Atelectasis"
cons_path = "D:\\ChestX-Det-Dataset-Lung-Top2"

baseline_img = np.fromfile("Dataset\\ig_baseline.bin", dtype=np.float32)
baseline_img.shape = 3, 224, 224

test_csv = pd.read_csv("Dataset\\mul_test.csv")
test_csv = test_csv.iloc[np.where(test_csv.loc[:, 'ate'] == 1)]
...
VGG11_optimal = torch.load("Model\\ate-VGG11.pt")
...
iou_list_test_case, dice_list_test_case, iou_case, dice_case = \
    explain_cases_baseline(test_loader_seg=test_cases_loader_bsl, device=device,
                           cls_model=VGG11_optimal, threshold=95, cat_id=1, baseline_img=baseline_img)
...
cv2.imwrite('Figure\\IG\\ATE-IG-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\IG\\ATE-IG-Mask.png')
cons = cv2.imread('D:\\ChestX-Det-Dataset-Lung-Top2' + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\IG\\ATE-IG-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    binary_list_iou = integrated_gradient_org(model=VGG11_optimal, image=images_test,
                                              baseline_img=baseline_img, steps=20, idx=1)
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\IG\\ATE-IG-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\IG\\ATE-IG-PRO.png')
