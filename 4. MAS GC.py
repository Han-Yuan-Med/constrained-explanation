...
cv2.imwrite('Figure\\GC\\mas-GC-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\mas-GC-Mask.png')
cons = cv2.imread(cons_path + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\GC\\mas-GC-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\mas-GC-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\mas-GC-PRO.png')
