...
# train_csv = pd.read_csv("Dataset\\negative.csv")
# train_set = Lung_cls(csv_file=train_csv, img_dir=image_path)
# baseline_img = baseline(train_set)
# baseline_img = baseline_img.detach().cpu().numpy()
# baseline_img.tofile("Dataset\\ig_baseline.bin")
...
results_df = []
#
# Alex_optimal = torch.load("model\\ate-AlexNet.pt")
#
...
#                            cls_model=Alex_optimal, threshold=95, baseline_img=baseline_img)
#
...
iou_list_test_case, dice_list_test_case, iou_case, dice_case = \
    explain_cases_baseline(test_loader_seg=test_cases_loader_bsl, device=device,
                           cls_model=VGG11_optimal, threshold=95, baseline_img=baseline_img)
...
Res18_optimal = torch.load("model\\ate-Res18.pt")

iou_list_test_case, dice_list_test_case, iou_case, dice_case = \
    explain_cases_baseline(test_loader_seg=test_cases_loader_bsl, device=device,
                           cls_model=Res18_optimal, threshold=95, baseline_img=baseline_img)
...
pd.DataFrame(results_df).to_csv("Result\\ate_ig.csv", index=False, encoding="cp1252")
