...
mask_path = "D:\\ChestX-Det10-Dataset-Mask\\Fracture"
...
test_csv = test_csv.iloc[np.where(test_csv.loc[:, 'fra'] == 1)]
...
VGG11_optimal = torch.load("Model\\fra-VGG11.pt")
...
cv2.imwrite('Figure\\IG\\fra-IG-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\IG\\fra-IG-Mask.png')
cons = cv2.imread('D:\\ChestX-Det-Dataset-Lung-Top2' + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\IG\\fra-IG-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\IG\\fra-IG-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\IG\\fra-IG-PRO.png')
