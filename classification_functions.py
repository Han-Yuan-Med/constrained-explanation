...
class Lung_cls_mul(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        image = cv2.resize(image, (224, 224))
        label = torch.tensor([int(self.annotations.iloc[index, 1]), int(self.annotations.iloc[index, 2]),
                              int(self.annotations.iloc[index, 3]), int(self.annotations.iloc[index, 4]),
...
class Lung_seg_cons(Dataset):
    def __init__(self, csv_file, img_dir, mask_dir, cons_dir):
        self.annotations = csv_file
        self.img_dir = img_dir
        self.mask_dir = mask_dir
    ...
    def __getitem__(self, index):
        ...
        cons_path_tmp = os.path.join(self.cons_dir, self.annotations.iloc[index, 0])
        cons = cv2.imread(cons_path_tmp, cv2.IMREAD_GRAYSCALE)
        cons[np.where(cons != 0)] = 1

        return image_fused, mask, cons
