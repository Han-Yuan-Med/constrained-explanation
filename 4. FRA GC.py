...
idx = np.argsort(np.asarray(iou_list_test_case_msk).astype(np.float32)-
          np.asarray(iou_list_test_case).astype(np.float32))[-4]
...
cv2.imwrite('Figure\\GC\\fra-GC-Image.png', cv2.resize(image, (224, 224)))
Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\fra-GC-Mask.png')
cons = cv2.imread('D:\\ChestX-Det-Dataset-Lung-Top2' + "\\" + test_csv.iloc[idx, 0], cv2.IMREAD_UNCHANGED)
cv2.imwrite('Figure\\GC\\fra-GC-cons.png', cons)
...
for data_test in tqdm(test_cases_loader_cons):
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\fra-GC-BSL.png')
    ...
    Image.fromarray((image_mask * 255).astype(np.uint8)).save('Figure\\GC\\fra-GC-PRO.png')
